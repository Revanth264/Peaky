...
import { generateDummyProducts, getNewArrivalsProducts } from '@/lib/dummy-products'
...
export default function NewArrivals() {
  ...
  const fetchNewArrivals = async () => {
    try {
      ...
      const response = await axios.get('/api/products?sort=createdAt:desc&limit=8').catch(() => null)
      ...
      if (response?.data?.products && response.data.products.length > 0) {
        ...
        setProducts(getNewArrivalsProducts(dummyProducts))
      }
    } catch (error) {

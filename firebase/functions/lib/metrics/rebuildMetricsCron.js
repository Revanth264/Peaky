...
async function rebuildMetrics() {
    ...
    for (const product of products) {
        const productRef = admin_js_1.db.collection('products').doc(product.productId);
    ...
    for (const [listName, ids] of Object.entries(materializedLists)) {
        const listRef = admin_js_1.db.collection('materialized').doc('home').collection(listName).doc('current');
        const list = {
            ids,
            updatedAt: (0, admin_js_2.serverTimestamp)(),
        };
...
//# sourceMappingURL=rebuildMetricsCron.js.map